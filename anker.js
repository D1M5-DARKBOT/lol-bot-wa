...
const { BarBarApi, ZeksApi, TechApi, TobzApi, VthearApi, LolHuman } = JSON.parse(fs.readFileSync('./database/json/apikey.json'))
...
const { help, bahasa, donasi, limitend, limitcount, bottt, casee } = require('./4NK3R/help')
...
async function starts() {
	...
	dms.on('chat-update', async (mek) => {
		try {
			...
			const BotWea2 = ["6281368646011@s.whatsapp.net"] // ubah aja gapapa
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		if (messagesLink.includes("anjing")){
		if (!isGroup) return
		...
		setTimeout( () => {
		reply(`[ Toxic Deteckted ] maaf *${pushname2}* anda akan di kick`)
		}, 0)
		}
		...
		reply(`[ Toxic Deteckted ] maaf *${pushname2}* anda akan di kick`)
			...
			let authorname = dms.contacts[from] != undefined ? dms.contacts[from].vname || dms.contacts[from].notify : undefined	
		...
		case prefix+'daftar':
			...
			//if (!isPublic) return reply(mess.only.publikG)
				...
				dms.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
				fs.unlinkSync(ran)
				})
				break
case prefix+'fast':
			...
			//if (!isPublic) return reply(mess.only.publikG)
				if (!isUser) return reply(mess.only.userB)
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
						...
						dms.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
						fs.unlinkSync(ran)
					})
				break
				case prefix+'gemuk':
				...
				case prefix+'s':
                    ...
                    case prefix+'stickerwm':
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('DARKBOT', authorname)} ${ran} -o ${ran}`, async (error) => {
								...
								/*dms.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)*/
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
								exec(`webpmux -set exif ${addMetadata('DARKBOT', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
									dms.sendMessage(from, fs.readFileSync(ranw), sticker, {quoted: mek})
									fs.unlinkSync(ranw)
								})
								//dms.sendMessage(from, fs.readFileSync(ranw), sticker, {quoted: mek})
							})
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           dms.sendMessage(from, options, text, { quoted: mek } )
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				    ...
				    kode = ts.split("/")[0]
				    teks = ts.split("/")[1]
				    anu = await fetchJson(`https://lolhuman.herokuapp.com/api/translate/auto/${kode}?apikey=${LolHuman}&text=${teks}`)
				    reply(mess.wait)
				    translate = `Text Asli: *${body.slice(11)}*\n\nHasil: *${anu.result.translated}*`
         ...
         case prefix+'liric':
                ...
                reply(mess.wait)
					teks = body.slice(7)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/lirik?apikey=${LolHuman}&query=${teks}`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				reply(mess.wait)
					memein = await getBuffer(`https://lolhuman.herokuapp.com/api/meme/memeindo?apikey=${LolHuman}`)
					dms.sendMessage(from, memein, image, {quoted: mek, caption: '.......'})
					await limitAdd(sender)
					break 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					teks = `${body.slice(7)}`
					reply(mess.wait)
					anuss = await getBuffer(`https://lolhuman.herokuapp.com/api/sswebfull?url=${teks}&apikey=${LolHuman}`)
					dms.sendMessage(from, anuss, image, {quoted: mek})
					await limitAdd(sender)
					break 
				case prefix+'nsfwloli':
				    ...
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						...
						res = await getBuffer(`https://lolhuman.herokuapp.com/api/random/nsfw/loli?apikey=${LolHuman}`, {method: 'get'})
						dms.sendMessage(from, res, image, {quoted: mek, caption: 'Nih Ajg Eh Anjim'})
					} catch (e) {
				...
				reply(mess.wait)
					...
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/hilih?apikey=${LolHuman}&text=${body.slice(7)}`, {method: 'get'})
				...
				reply(mess.wait)
					if (args.length < 1) return reply('Teksnya mana gan?')
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/alay?apikey=${LolHuman}&text=${body.slice(6)}`, {method: 'get'})
					dms.sendMessage(from, `${anu.result}`, text, {quoted: mek})
					await limitAdd(sender) 
					break 
				...
				reply(mess.wait)
					if (args.length < 1) return reply('Teksnya mana gan?')
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/ninja?apikey=${LolHuman}&nama=${body.slice(11)}`, {method: 'get'})
					dms.sendMessage(from, `${anu.result}`, text, {quoted: mek})
					await limitAdd(sender) 
					break 
				case prefix+'chord':
               ...
               if (isLimit(sender)) return reply(limitend(pushname2))
                   ...
                   await limitAdd(sender) 
                   break 
                case prefix+'jumlahair':
               if (isBanned) return reply(mess.only.benned)    
               if (!isUser) return reply(mess.only.userB)
               //if (!isPublic) return reply(mess.only.publikG)
               if (isLimit(sender)) return reply(limitend(pushname2))
                   anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/cuaca/air-bagi-tanaman?apikey=freeKeY`, {method: 'get'})
                   if (anu.error) return reply(anu.error)
                   buff = await getBuffer(anu.result.image)
                   reply(mess.wait)
                   airr = `•Deskripsi *${anu.result.desc}*`
                   dms.sendMessage(from, buff, image, {quoted: mek, caption: airr})
                   await limitAdd(sender) 
                   break 
                case prefix+'kucing':
                if (isBanned) return reply(mess.only.benned)    
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/pinterest?apikey=${LolHuman}&query=kucing`, {method: 'get'})
					reply(mess.wait)
...
// only grup fitur anime
              case prefix+'anime':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					reply(mess.wait)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/pinterest?apikey=${LolHuman}&query=kaneki`, {method: 'get'})
					ki = await getBuffer(anu.result)
...
// akhir fitur anime
               ...
               case prefix+'cersex':
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                if (isLimit(sender)) return reply(limitend(pushname2))
                   anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/cersex?apikey=freeKeY`, {method: 'get'})
                   if (anu.error) return reply(anu.error)
                   sex = await getBuffer(anu.gambar)
                   reply (mess.wait)
                   cerita = `• *Cersex:* ${anu.result}`
              ...
              case prefix+'randomkpop':
               ...
               case prefix+'shopee':
                ...
                if (isLimit(sender)) return reply(limitend(pushname2)) 
                pshhk = `${body.slice(8)}`
                  anu = await fetchJson(`https://pencarikode.xyz/api/shopee?search=${pshhk}&apikey=pais`, {method: 'get'})
                  pureek = '[+]××××××××××××××××××[+]\n\n'
                  for (let plylo of anu.result){
                  pureek += `• *Nama:* ${plylo.nama}\n• *Harga:* ${plylo.harga}• *Terjual:* ${plylo.terjual}\n• *Lokasi:* ${plylo.lokasi}\n• *Link Produk:* ${plylo.link_produk}\n• *Deskripsi:* ${plylo.deskripsi}\n\n[+]××××××××××××××××××××[+]\n\n`
                  }
                  reply(pureek.trim())
                  break
               case prefix+'pornhub':
			   ...
			   reply(mess.wait)
				...
				reply(mess.wait)
					...
					reply(mess.wait)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/facebook?apikey=${LolHuman}&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
			...
			/*case prefix+'instaimg':
				...
				anu = await fetchJson(`https://lolhuman.herokuapp.com/api/igstory/${instor}?apikey=${LolHuman}`, {method: 'get'})
				insta = '=========================\n\n'
				for (let ik of anu.result) {
				insta += `• *Type:* ${ik.type}\n• *Link:* ${ik.url}\n\n=========================\n\n`
				}
				reply(insta.trim())
				await limitAdd(sender)
			...
			case prefix+'hekerbucin':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/ytsearch?apikey=${LolHuman}&query=${body.slice(10)}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = '=================\n\n'
			    ...
			    //if (!isPublic) return reply(mess.only.publikG)
                    ...
                    break
//creator
				case prefix+'ttp': //Anker Rvan Kasih Credit Ajg
							pngttp = './temp/ttp.png'
							...
							dms.sendMessage(from, buffer, sticker)
				...
				case prefix+'goldbutton':
                     ...
                     if (args.length > 10) return reply('karakter minimal 10')
					 gbu = `${body.slice(12)}`
					 buff = await getBuffer(`https://lolhuman.herokuapp.com/api/ephoto1/goldplaybutton?apikey=${LolHuman}&text=${gbu}`, {method: 'get'})
					 dms.sendMessage(from, buff, image, {quoted: mek})
					 await limitAdd(sender)
					 break 
				case prefix+'silverbutton':
                     ...
                     if (args.length > 10) return reply('karakter minimal 10')
					 tsil = `${body.slice(14)}`
					 buff = await getBuffer(`https://lolhuman.herokuapp.com/api/ephoto1/silverplaybutton?apikey=${LolHuman}&text=${tsil}`, {method: 'get'})
					 dms.sendMessage(from, buff, image, {quoted: mek})
					 await limitAdd(sender)
					 break 
				case prefix+'tahta':
                ...
                if (isBanned) return reply(mess.only.benned)
                tahtay = `${body.slice(7)}`
                     buff = await getBuffer(`https://lolhuman.herokuapp.com/api/nulis?apikey=${LolHuman}&text=${tahtay}`, {method: 'get'})
                     dms.sendMessage(from, buff, image, {quoted: mek, caption: `Nih Anjim`})
                  await limitAdd(sender) 
                  break  
                case prefix+'glitch':
...
//akhir kreator
			...
			case prefix+'pasangan':
				...
				sa = pa.split("/")[0];
				ngan = pa.split("/")[1];
                      pass = await fetchJson(`https://lolhuman.herokuapp.com/api/jodoh/${sa}/${ngan}?apikey=${LolHuman}`)
                     buffer = await getBuffer(pass.result.image)
                     saan = `*Positif* : *_${pass.result.positif}_*\n*Negatif* : *_${pass.result.negatif}_*\n*Deskripsi* : *_${pass.result.deskripsi}_*`
                    dms.sendMessage(from, buffer, image, {quoted: mek, caption: saan})
                    await limitAdd(sender)
                    break
			   case prefix+'igstalk':
                      if (isBanned) return reply(mess.only.benned)
			    ...
			    if (!isUser) return reply(mess.only.userB)
			    //if (!isPublic) return reply(mess.only.publikG)
                      hkmm = await fetchJson(`https://leyscoders-api.herokuapp.com/api/nabi?q=${body.slice(11)}&apikey=freeKeY`)
                     buffer = await getBuffer(hkmm.result.img)
                     hasilj = `*Nama* : *_${hkmm.result.nama}_*\n*Lahir* : *_${hkmm.result.lahir_pada}_*\n*Umur* : *_${hkmm.result.umur}_*\n*Tempat* : ${hkmm.result.tempat}\n*Kisah* : *_${hkmm.result.kisah}_*`
                    dms.sendMessage(from, buffer, image, {quoted: mek, caption: hasilj})
                    await limitAdd(sender)
                    break
                 case prefix+'tahta':
                if (isBanned) return reply(mess.only.benned)
			    ...
			    //if (!isPublic) return reply(mess.only.publikG)
			    reply(mess.wait)
			        anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/artimimpi?q=${body.slice(7)}&apikey=freeKeY`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/faktaunik?apikey=${LolHuman}`, {method: 'get'})
				...
				//if (!isPublic) return reply(mess.only.publikG)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/faktaunik?apikey=${LolHuman}`, {method: 'get'})
					faktaun = `Faktanya: *${anu.result}*`
					dms.sendMessage(from, faktaun, text, {quoted: mek})
					await limitAdd(sender) 
					break 
				case prefix+'quotes2':
				if (isBanned) return reply(mess.only.benned)   
				...
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/quotes?apikey=${LolHuman}`, {method: 'get'})
					faktaa = `*By* : *${anu.result.by}*\n*Quotes* : *${anu.result.quote}*`
				...
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/quotesnime?apikey=${LolHuman}`, {method: 'get'})
					anin = `*Quote* : *${anu.result.quote}*\n*Character* : *${anu.result.character}*\n*Anime* : *${anu.result.anime}*`
					dms.sendMessage(from, anin, text, {quoted: mek})
					await limitAdd(sender) 
					break 
				...
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/quotes/dilan?apikey=${LolHuman}`, {method: 'get'})
					dill = `*Author* : *Dilane*\n*Quotes* : *${anu.result}*`
					dms.sendMessage(from, dill, text, {quoted: mek})
					await limitAdd(sender) 
					break 
				...
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/nama?apikey=${LolHuman}`, {method: 'get'})
					dillb = `*Nama* : *${anu.result}*`
					dms.sendMessage(from, dillb, text, {quoted: mek})
					await limitAdd(sender) 
					break 
				case prefix+'cerpen':
				if (isBanned) return reply(mess.only.benned)   
				...
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/cerpen?apikey=${LolHuman}`, {method: 'get'})
					dil2 = `*Judul* : *${anu.result.title}*\n*Creator* : *${anu.result.creator}*\n*Cerpen* : *${anu.result.cerpen}*`
					dms.sendMessage(from, dil2, text, {quoted: mek})
					await limitAdd(sender) 
					break 
				case prefix+'fml':
				if (isBanned) return reply(mess.only.benned)   
				...
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
					anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/skak?apikey=freeKeY`, {method: 'get'})
					fmli = `*Sindiran* : *${anu.result}*`
					dms.sendMessage(from, fmli, text, {quoted: mek})
					await limitAdd(sender) 
					break 
				case prefix+'tahilalat':
				if (isBanned) return reply(mess.only.benned)   
				 if (isLimit(sender)) return reply(limitend(pushname2))
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
					anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/tahi-lalat?apikey=freeKeY`, {method: 'get'})
					fmlil = `*Isi* : *${anu.result}*`
					dms.sendMessage(from, fmlil, text, {quoted: mek})
					await limitAdd(sender) 
					break 
			...
			case prefix+'motivasi':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					dms.sendMessage(from, mott, text, {quoted: mek})
					await limitAdd(sender) 
					break 
			case prefix+'htmlscraper':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://pencarikode.xyz/html?url=${body.slice(13)}&apikey=pais`, {method: 'get'})
					dms.sendMessage(from, `${anu.html}`, text, { quoted: mek})
					await limitAdd(sender) 
					break 
		    ...
		    case prefix+'darkjokes':
				...
				//if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./4NK3R/drak.js');
			...
			case prefix+'katacinta':
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
				anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/bucin?apikey=${LolHuman}`, {method: 'get'})
				katacin = `*${anu.result}*`
		    ...
		    case prefix+'map':
                ...
                case prefix+'randomkick':
					...
					const sipsss = pushname2[Math.floor(Math.random() * msls.length)]
				...
				if (!isGroup) return reply(mess.only.group)
					...
					teks = body.slice(6)
					anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/simi?kata=${teks}&apikey=freeKeY`, {method: 'get'})
					simii = `*${anu.simi}*`
				...
				if (!isGroup) return reply(mess.only.group)
					...
					if (args.length < 1) return reply('ketik !antilink on untuk mengaktifkan')
					if ((args[0]) === 'on') {
						if (isAntiToxic) return reply('anti link sudah on')
						antitoxic.push(from)
						fs.writeFileSync('./database/json/antitoxic.json', JSON.stringify(antilink))
						reply(`\`\`\`✓Sukses mengaktifkan fitur anti link di group\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						if (!isAntiToxic) return reply('anti link sudah off')
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/caklontong?apikey=${LolHuman}`, {method: 'get'})
					caklontong = `*${anu.result.question}*`
					setTimeout( () => {
					dms.sendMessage(from, '*➸ Jawaban :* *'+anu.result.answer+ '*', text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/tebak/family100?apikey=${LolHuman}`, {method: 'get'})
					family = `*${anu.result.question}*`
					setTimeout( () => {
					dms.sendMessage(from, '*➸ Jawaban :* '+anu.result.aswer, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					dms.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
				...
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
				 data = fs.readFileSync('./4NK3R/18.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
     ...
     reply(mess.wait)
         ...
         if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					pinte = body.slice(11)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/pinterest?apikey=${LolHuman}&query=${pinte}`, {method: 'get'})
					reply(mess.wait)
		...
		case prefix+'wallpaper':
         if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return reply('Mau Nyari Foto Apa???')
					pintee = body.slice(11)
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/wallpaper?apikey=${LolHuman}&query=${pintee}`, {method: 'get'})
					reply(mess.wait)
					pinehge = await getBuffer(anu.result)
					dms.sendMessage(from, pinehge, image, { caption: '*Wallpaper*\n\n*Hasil Pencarian : '+pintee+'*', quoted: mek })
					await limitAdd(sender) 
					break 
		case prefix+'loli':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/loli?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					loll = await getBuffer(anu.result)
					dms.sendMessage(from, loll, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
		case prefix+'neko':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/neko?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					loll1 = await getBuffer(anu.result)
					dms.sendMessage(from, loll1, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
        case prefix+'waifu':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/waifu?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					loll2 = await getBuffer(anu.result)
					dms.sendMessage(from, loll2, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
		case prefix+'shota':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/shota?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					loll3 = await getBuffer(anu.result)
					dms.sendMessage(from, loll3, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
		case prefix+'husbu':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/husbu?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					loll4 = await getBuffer(anu.result)
					dms.sendMessage(from, loll4, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
		case prefix+'sagiri':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/sagiri?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					loll5 = await getBuffer(anu.result)
					dms.sendMessage(from, loll5, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
		case prefix+'shinobu':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/shinobu?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					loll6 = await getBuffer(anu.result)
					dms.sendMessage(from, loll6, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
		case prefix+'megumin':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/megumin?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					loll7 = await getBuffer(anu.result)
					dms.sendMessage(from, loll7, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
		case prefix+'wallnime':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/random/wallnime?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					loll8 = await getBuffer(anu.result)
					dms.sendMessage(from, loll8, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
		case prefix+'quotes':
         if (isBanned) return reply(mess.only.benned)    
				...
				//if (!isPublic) return reply(mess.only.publikG)
				if (isLimit(sender)) return reply(limitend(pushname2))
					quo = await getBuffer(`https://lolhuman.herokuapp.com/api/random/quotesimage?apikey=${LolHuman}`, {method: 'get'})
					reply(mess.wait)
					dms.sendMessage(from, quo, image, { caption: '*Nih Anjim*', quoted: mek })
					await limitAdd(sender) 
					break 
		...
		case prefix+'jadwalsholat':
			...
			case prefix+'jadwaltv':
				...
				//if (!isPublic) return reply(mess.only.publikG)
					...
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/jadwaltv/${jadwaltv}?apikey=${LolHuman}`, {method: 'get'})
           ...
           case prefix+'jadwaltvnow':
                ...
                //if (!isPublic) return reply(mess.only.publikG)
                if (isLimit(sender)) return reply(limitend(pushname2))
                anu = await fetchJson(`https://lolhuman.herokuapp.com/api/jadwaltv/now?apikey=${LolHuman}`, {method: 'get'})
                tvnow = `*Antv* : *${anu.result.antv}*\n*Gtv* : *${anu.result.gtv}*\n*Indosiar* : *${anu.result.indosiar}*\n*Mnctv* : *${anu.result.mnctv}*\n*Rcti* : *${anu.result.rcti}*`
...
// premium user
       ...
       case prefix+'ytmp4':
					...
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/ytvideo?apikey=${LolHuman}&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					ytt = `╭─「 *YOUTUBE MP4 DOWNLOADER* 」\n│\n│• *Title:* ${anu.result.title}\n│• *Size:* ${anu.result.link[0].size}\n│\n│ Tunggu Sebentar 1 menit Mungkin Agak Lama \n│ Karna Mendownload Video\n╰─────────────────────`
				...
				case prefix+'ytmp3':
					...
					anu = await fetchJson(`https://lolhuman.herokuapp.com/api/ytaudio?apikey=${LolHuman}&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					yta = `╭─「 *YOUTUBE MP3 DOWNLOADER* 」\n│\n│• *Title:* ${anu.result.title}\n│• *Size:* ${anu.result.link[3].size}\n│\n│ Tunggu Sebentar 1 menit Mungkin Agak Lama \n│ Karna Mendownload Video\n╰─────────────────────`
					buff = await getBuffer(anu.result.thumbnail)
					reply(mess.wait)
					buffer = await getBuffer(anu.result.link[3].link)
           ...
           case prefix+'play':
            ...
            case prefix+'citacita':
                ...
                dms.sendMessage(from, anu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                await limitAdd(sender) 
                break 
...
case prefix+'asupan':
				...
				//if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./4NK3R/asupan.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
		...
		case prefix+'bitly':
			...
			dms.sendMessage(from, `*Pantun* : *_${anu.pantun}_*`, text, {quoted: mek})
			await limitAdd(sender) 
			break  
...
// akhir encrypt & decrypt Fitur
         ...
         case prefix+'antidelete':
				...
				if (argz.length === 1) return dms.sendMessage(from, `Penggunaan fitur antidelete :\n\n*${prefix}antidelete [aktif/mati]* (Untuk grup)\n*${prefix}antidelete [ctaktif/ctmati]* (untuk semua kontak)\n*${prefix}antidelete banct 628558xxxxxxx* (banlist kontak)`, MessageType.text)
				if (argz[1] == 'aktif') {
					if (isGroup) {
						if (isRevoke) return dms.sendMessage(from, `Antidelete telah diaktifkan di grup ini sebelumnya!`, MessageType.text)
						dataRevoke.push(from)
						fs.writeFileSync('./lib/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						dms.sendMessage(from, `*Succes Enable Antidelete Grup!*`, MessageType.text)
					} else if (!isGroup) {
						dms.sendMessage(from, `Untuk kontak penggunaan *${prefix}antidelete ctaktif*`, MessageType.text)
					}
				} else if (argz[1] == 'ctaktif') {
					if (!isGroup) {
						if (isCtRevoke) return dms.sendMessage(from, `Antidelete telah diaktifkan di semua kontak sebelumnya!`, MessageType.text)
						dataCtRevoke.data = true
						fs.writeFileSync('./lib/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						dms.sendMessage(from, `Antidelete diaktifkan disemua kontak!`, MessageType.text)
					} else if (isGroup) {
						dms.sendMessage(from, `Untuk grup penggunaan *${prefix}antidelete aktif*`, MessageType.text)
					}
				} else if (argz[1] == 'banct') {
					if (isBanCtRevoke) return dms.sendMessage(from, `kontak ini telah ada di database banlist!`, MessageType.text)
					if (argz.length === 2 || argz[2].startsWith('0')) return dms.sendMessage(from, `Masukan nomer diawali dengan 62! contoh 6285928xxxxx`, MessageType.text)
					dataBanCtRevoke.push(argz[2] + '@s.whatsapp.net')
					fs.writeFileSync('./lib/ct-revoked-banlist.json', JSON.stringify(dataBanCtRevoke, null, 2))
					dms.sendMessage(from, `Kontak ${argz[2]} telah dimasukan ke banlist antidelete secara permanen!`, MessageType.text)
				} else if (argz[1] == 'mati') {
					if (isGroup) {
						const index = dataRevoke.indexOf(from)
						dataRevoke.splice(index, 1)
						fs.writeFileSync('./lib/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						dms.sendMessage(from, `*Succes disable Antidelete Grup!*`, MessageType.text)
					} else if (!isGroup) {
						dms.sendMessage(from, `Untuk kontak penggunaan *${prefix}antidelete ctmati*`, MessageType.text)
					}
				} else if (argz[1] == 'ctmati') {
					if (!isGroup) {
						dataCtRevoke.data = false
						fs.writeFileSync('./lib/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						dms.sendMessage(from, `Antidelete dimatikan disemua kontak!`, MessageType.text)
					} else if (isGroup) {
						dms.sendMessage(from, `Untuk grup penggunaan *${prefix}antidelete mati*`, MessageType.text)
					}
				}
				break
